...
def setup_logger(level: str = "INFO") -> logging.Logger:
    """Configura logging con colores y trazas limpias."""
    logging.basicConfig(
...
def log_history(entry: dict) -> None:
    """Guarda una acción en el historial para permitir --undo."""
    history = []
...
def execute_action(file: Path, target: Path, action: str, dry_run: bool) -> bool:
    """Ejecuta move/copy/symlink o simula en dry-run."""
    if dry_run:
    ...
    if action == "move":
        ...
        console.print(f"⚠️ Acción desconocida: {action}")
        return False
    return True

def rollback_history() -> bool:
    """Revierte la última ejecución registrada."""
    if not HISTORY_FILE.exists():
        console.print("ℹ️ Nada que deshacer.")
        return False
    
    try:
        history = json.loads(HISTORY_FILE.read_text())
    except json.JSONDecodeError:
        console.print("⚠️ Historial corrupto. Borrando archivo.")
        HISTORY_FILE.unlink()
        return False
    ...
    HISTORY_FILE.unlink()
    console.print("↩️ Rollback completado.")
    return True